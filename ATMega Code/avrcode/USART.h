...
void printLongNumber(uint32_t longWord); 
		/* Prints a (32 -bits) out as 10-digit ascii equivalent */
		
void printLong(unsigned long longNum);
		/* Prints a long (32-bits) out as raw binary */
		
void printLongLong(unsigned long long longNum);
		/* Prints a long (64-bits) out as raw binary */
...
/* takes in up to 5 ascii digits,
 converts them to a uint16_t byte when press enter */

uint32_t getNumber24(void);
/* takes in up to 6 ascii digits,
 converts them to a 32 uint_32t byte when press enter */
